...
export default function ZoneTrends({ jobs, zones }: ZoneTrendsProps) {
  ...
  // When "new sites" toggled on, fetch all historical delivery postcodes before the 3-month window
  useEffect(() => {
  ...
  const { postcodeTrends, zoneTrends, monthLabels } = useMemo(() => {
    ...
    // When newSitesOnly, only include delivery jobs to prefixes NOT in historicalPrefixes
