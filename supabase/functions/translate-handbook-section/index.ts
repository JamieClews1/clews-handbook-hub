...
async function translateText(texts: string[], targetLang: string, deeplApiKey: string, retries = 3): Promise<string[]> {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    await new Promise(resolve => setTimeout(resolve, 1500)); // 1.5s between successful requests
...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey || !supabaseAnonKey) throw new Error('Supabase credentials not configured');
